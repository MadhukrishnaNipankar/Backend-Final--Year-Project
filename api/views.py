        ...
        # else create the account
            try:
                ...
                try:
                    ...
                    loggedinStatusObject = LoginStatus(
                        is_loggedin=False, user=userObject)
        ...
        if request.method == "POST":
            ...
            if(LoginStatusObject.is_loggedin == True):  # verifying if user is logged in
                ...
                for historyItem in HistoryObjects:
                    VideoObject = VideoData.objects.get(
                        sno=historyItem.video_id)
...
def loginStatus(request):
    try:
        if request.method == "POST":
            ...
            userObject = User.objects.get(email=email)
            LoginStatusValue = LoginStatus.objects.get(
                user=userObject).is_loggedin
    ...
    try:
        if request.method == "POST":
            ...
            if(LoginStatusObject.is_loggedin == True):  # verifying if user is logged in
                ...
                profile_photoObject = UserProfilePhoto.objects.get(
                    user=userObject)
        ...
        if request.method == "POST":
            ...
            if(LoginStatusObject.is_loggedin == True):  # verifying if user is logged in
                ...
                QuickNotesObj = QuickNotes(
                    notes_value=notes_value, user=userObject,video_id=video_id)
                QuickNotesObj.save()
...
def getQuickNotes(request):
    try:
        if request.method == "POST":
            ...
            if(LoginStatusObject.is_loggedin == True):  # verifying if user is logged in
                # getting quick notes
                QuickNotesObj = QuickNotes.objects.filter(user=userObject,video_id=video_id)
                if((len(QuickNotesObj)) == 0):
                    serializer = QuickNotesSerializer(QuickNotesObj, many=True)
                    responseObject = {
                    "status": 404,
                    "response":"You haven't saved any notes for this video"
                     }
                    json_data = JSONRenderer().render(responseObject)
                    return HttpResponse(json_data, content_type='application/json')
